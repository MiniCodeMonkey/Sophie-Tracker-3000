...
return;$(".gridster ul").gridster({widget_margins:[10,10],widget_base_dimensions:[250,250]});$.get('stats/update',function(data){$('.profile-age').html(data.profile.age+' old');$('.profile-hygiene').css('width',(data.profile.attributes.hygiene*100.0)+'%');$('.profile-hunger').css('width',(data.profile.attributes.hunger*100.0)+'%');$('.profile-bladder').css('width',(data.profile.attributes.bladder*100.0)+'%');$('.profile-energy').css('width',(data.profile.attributes.energy*100.0)+'%');if(data.profile.sleeping){$('.box-profile .sleep-items').removeClass('hide');$('.profile-age').append('<span>Sleeping (ssh!)</span>');$('.profile-energy').parent().addClass('active');}else{$('.box-profile .sleep-items').addClass('hide');$('.profile-energy').parent().removeClass('active');}
...
$('.last-fed-amount').html(amount);$('.feed-time-next').html(data.feed_time.next_feed_formatted);$.each(data.day_chart,function(index,event){var colorName=null;switch(event.type){case'Feed':colorName='info';break;case'Diaper':colorName='success';break;case'Sleep':colorName='warning';break;case'Activity':colorName='warning';break;case'Medicine':colorName='danger';break;case'Bath':colorName='info';break;}
