...
{$.get('track/stats',function(response){$('.eventbutton-feed').find('.badge').html(response.feed.time);$('.eventbutton-pump').find('.badge').html(response.pump.time);$('.eventbutton-diaper').find('.badge').html(response.diaper.time);if(response.sleep.type=='start'){if($('.sleep-items').hasClass('hide')){$('.sleep-items').removeClass('hide');$('body, .face.front').animate({backgroundColor:'#D8D8D8'});}}else{if(!$('.sleep-items').hasClass('hide')){$('.sleep-items').addClass('hide');$('body, .face.front').animate({backgroundColor:'#FFF'});}}});}
$(function(){$('#medicineModal .medicine-types button').fastClick(function(){var medicineType;if($(this).data('value')=='Other'){medicineType=prompt('Enter name:');}else{medicineType=$(this).data('value');}
trackEvent($('#medicineModal'),'Medicine',medicineType);return false;});$('#medicineModal button.save').fastClick(function(){trackEvent($('#medicineModal'),'Medicine',medicineType);return false;});});$(function(){$('#milestoneModal button.save').fastClick(function(){var description=$('#milestoneModal textarea').val();if(description.length<=0){alert('Please enter a description');return false;}else if(description.length>255){alert('Max 255 characters, sorry!');return false;}
...
$('#success-notification .undo').fastClick(function(){deleteEvent($('#success-notification').data('event-id'));return false;});$(function(){var pumpType;$('#pumpModal').on('hidden.bs.modal',function(){$('#pumpModal .pump-types button').removeClass('btn-primary').removeClass('btn-info').addClass('btn-info');$('#pumpModal .pump-options').addClass('hide');});$('#pumpModal .pump-types button').fastClick(function(){$('#pumpModal .pump-types button').removeClass('btn-primary').removeClass('btn-info').addClass('btn-info');$(this).removeClass('btn-info').addClass('btn-primary');$('#pumpModal .pump-options').removeClass('hide');pumpType=$(this).data('value');return false;});$('#pumpModal button.save').fastClick(function(){trackEvent($('#pumpModal'),'Pump',pumpType,$(this).parent().find('.spinner').data('value'));return false;});});$(function(){$('.eventbutton-sleep').fastClick(function(){trackEvent(null,'Sleep');return false;});});$(function(){$('.spinner .spinner-left').fastClick(function(){var currentValue=$(this).parent().data('value');var step=$(this).parent().data('step');var newValue=currentValue-=step;if(newValue>0){$(this).parent().data('value',newValue);$(this).parent().find('.amount').html(formatDecimal(newValue));}});$('.spinner .spinner-right').fastClick(function(){var currentValue=$(this).parent().data('value');var step=$(this).parent().data('step');var newValue=currentValue+=step;$(this).parent().data('value',newValue);$(this).parent().find('.amount').html(formatDecimal(newValue));});});function formatDecimal(value){var integerPart=Math.floor(value);var decimalPart=value-integerPart;if(decimalPart==0.25){return integerPart+'&frac14;';}else if(decimalPart==0.5){return integerPart+'&frac12;';}else if(decimalPart==0.75){return integerPart+'&frac13;';}else{return value;}}
$(function(){if(!$('body#stats').length)
return;$(".gridster ul").gridster({widget_margins:[10,10],widget_base_dimensions:[250,250]});$.get('stats/update',function(data){$('.profile-age').html(data.profile.age+' old');var ctx=$("#diaperchart").get(0).getContext("2d");var options={scaleOverride:true,scaleSteps:10,scaleStepWidth:2,scaleStartValue:0,scaleLabel:"<%=value%>",scaleFontColor:"#FFF",pointDotRadius:3,datasetStrokeWidth:1};var diaperchart=new Chart(ctx).Line(data.diaper_graph,options);$('.diapers-available').html(data.diaper_stats.available);$('.diapers-run-out-days').html(data.diaper_stats.run_out.days+'<span>days</span>');$('.diapers-run-out-date').html(data.diaper_stats.run_out.date);$('.diapers-average').html(Math.round(data.diaper_stats.used_per_day*10)/10);$('.last-fed-icon').removeClass().addClass('last-fed-icon').addClass(data.last_fed.icon);$('.last-fed-time').html(data.last_fed.formatted_time.replace(' and',', '));var amount='';if(data.last_fed.type=='left'||data.last_fed.type=='right'){amount=data.last_fed.value+'<span>min.</span>';}else{amount=data.last_fed.value+'<span>oz.</span>';}
$('.last-fed-amount').html(amount);$('.last-fed-type').html(data.last_fed.type);$('.feed-time-next').html(data.feed_time.next_feed_formatted);});});$(function(){var suppliesType;$('#suppliesModal').on('hidden.bs.modal',function(){$('#suppliesModal .supplies-types button.temporary').remove();$('#suppliesModal .supplies-types button').removeClass('btn-primary').removeClass('btn-info').addClass('btn-info');$('#suppliesModal .supplies-options').addClass('hide');});$('#suppliesModal .supplies-types button').fastClick(function(){$('#suppliesModal .supplies-types button').removeClass('btn-primary').removeClass('btn-info').addClass('btn-info');if($(this).data('value')=='other'){var name=prompt('Enter name:');var button=$('<button>').attr('type','button').addClass('btn').addClass('btn-lg').addClass('btn-primary').addClass('temporary').data('value',name).html('<i class="icon-star"></i> '+name);button.insertBefore($(this));suppliesType=name;}else{$(this).removeClass('btn-info').addClass('btn-primary');suppliesType=$(this).data('value');}
$('#suppliesModal .supplies-options').removeClass('hide');return false;});$('#suppliesModal button.save').fastClick(function(){trackEvent($('#suppliesModal'),'Supplies',suppliesType,$(this).parent().find('.spinner').data('value'));return false;});});function trackEvent(activeModal,type,subtype,value)
{type=type||'';subtype=subtype||'';value=value||'';if(activeModal){var spinner=$('<i>').addClass('icon-spinner').addClass('icon-spin').addClass('icon-2x');activeModal.find('.modal-body').append(spinner);}
...
activeModal.modal('hide');updateLastEvent();}else{showNotification(type,true);}});}
