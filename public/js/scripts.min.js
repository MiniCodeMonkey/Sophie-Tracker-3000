...
trackEvent($('#noteModal'),'Note',description);return false;});});function showNotification(event,error){error=error||false;var formattedEvent=formatEvent(event);var selector=error?'#error-notification':'#success-notification';$(selector+' .event-type').html(formattedEvent.description);$(selector).stop().css('top','-500px').show().animate({top:'0px'}).delay(8000).animate({top:'-500px'});}
